...
const GITHUB_REPO = 'EthanYan6/Syrup';
...
/** Dondji/Syrup CN font @ SPI Flash 0x024000（与 App/settings.h 一致） */
...
const CN_FONT_BIN_NAME    = 'cn_font.bin';
const CALIB_SIZE          = 512;
/** App/ui/welcome.c LOGO_FLASH_ADDR — Syrup uses 0x011000 (not Dondji 0x1FF000) */
...
/** 串口原始收包字节累计（不被 fetchMessage 清空逻辑影响，用于诊断） */
...
async function connect() {
  ...
  try {
    ...
    console.warn('setSignals(DTR/RTS) failed', sigErr);
  }
...
async function readLoop() {
  try {
    ...
    if (isReading) log(window.t ? window.t('logError', {msg: e.message}) : ('读取错误: ' + e.message), 'error');
  }
}
...
async function requestDeviceInfoForCalib(purpose) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    let tick = 0;
    const ticksPerAttempt = attempt === 1 ? 400 : 300; // 约 4s / 3s
    for (; tick < ticksPerAttempt; tick++) {
      await sleep(10);
      ...
      sawAnyMessage = true;
      const typeHex = resp.msgType.toString(16).padStart(4, '0');
      log(window.t ? window.t('logReceivedMessage', {type: typeHex}) : '收到消息: 0x' + typeHex, 'info');
      if (resp.msgType === MSG_NOTIFY_DEV_INFO) {
        sawBootloaderNotify = true;
        continue;
      }
    ...
    if (attempt < maxAttempts) {
      log('未收到 0x0515，重试设备信息请求（' + (attempt + 1) + '/' + maxAttempts + '）...', 'info');
      await sleep(200);
    }
  }

  const bytesLeft = readBuffer.length;
  ...
  if (!sawAnyMessage) {
    throw new Error(
      '超时：串口已连接但设备无协议应答（原始收到 ' +
        rawRx +
        ' 字节，解析缓冲 ' +
        bytesLeft +
        '）。请确认：①机内已刷带 USB 的 Syrup/同类固件且正常开机；②选的是对讲机 USB CDC 口不是其它 COM；③数据线能传数据（非仅充电）；④屏幕在点「备份校准」时是否闪一下变暗（变暗说明命令已到机）。也可先试叮咚鸡网页能否读到设备信息做对比。'
    );
  }
  throw new Error('超时：未收到设备信息 0x0515（请开机进入正常工作界面再试，勿停在纯 Bootloader 刷机界面）');
}
...
/**
 * 从 GitHub Releases 拉取最新 syrup_*.bin（本地 docs/firmware 优先，见 firmwareFetchArrayBufferWithFallback）。
 */
async function firmwareFetchLatestFromGitHubReleases() {
  ...
  for (let i = 0; i < assets.length; i++) {
    ...
    if (lower.endsWith('.bin') && lower.indexOf('syrup') !== -1) {
      chosen = assets[i];
      break;
    }
  }
...
async function firmwareFetchArrayBufferWithFallback() {
  // Prefer docs/firmware/syrup.bin (packaged with each build); GitHub Releases as fallback.
  try {
...
function cnFontFetchArrayBuffer() {
  ...
  function attemptNextUrl() {
    ...
    }).catch(function() { return attemptNextUrl(); });
  }
  return attemptNextUrl();
}
...
function cnFontParseCodepointsFromBin(arrayBuffer) {
  ...
  if (bytes.length < minBytes) {
    throw new Error('字库过短: ' + bytes.length + ' < ' + minBytes);
  }
  const codepointSet = new Set();
  const view = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
  for (let entryIndex = 0; entryIndex < CN_FONT_CHAR_COUNT; entryIndex++) {
    const byteOffset = CN_FONT_BITMAP_SIZE + entryIndex * 4;
    const codePoint = view.getUint16(byteOffset, true);
    if (codePoint !== 0 && codePoint !== 0xffff) codepointSet.add(codePoint);
  }
  return codepointSet;
}
...
function cnFontOnFontDataLoaded() {
  ...
  try {
    ...
    cnFontCodepointSetPromise = Promise.resolve(cnFontParseCodepointsFromBin(ab));
  } catch (parseErr) {
    console.warn('刷字库已加载数据无法用于缺字码表解析', parseErr);
  }
}

function cnFontGetCodepointSet() {
  if (cnFontCodepointSetPromise !== null) return cnFontCodepointSetPromise;
  cnFontOnFontDataLoaded();
  if (cnFontCodepointSetPromise !== null) return cnFontCodepointSetPromise;
  cnFontCodepointSetPromise = cnFontFetchArrayBuffer().then(function(arrayBuffer) {
    return cnFontParseCodepointsFromBin(arrayBuffer);
...
function cnFontSetUiLoaded(name, bytes) {
  const info = $('fontInfo');
  if (info) {
    ...
    info.innerHTML =
      ...
      CN_FONT_CHAR_COUNT + ' 字符 @ 0x024000';
  }
...
on('fontFile', 'change', function(e) {
  ...
  fr.onload = function(ev) {
    ...
    cnFontOnFontDataLoaded();
    cnFontSetUiLoaded(file.name, fontData);
...
on('fetchFontBtn', 'click', async function() {
  ...
  try {
    ...
    cnFontOnFontDataLoaded();
    cnFontSetUiLoaded(CN_FONT_BIN_NAME, fontData);
    log(window.t ? window.t('logFontLoadedDefault', {name: CN_FONT_BIN_NAME, size: fontData.length}) : '字库已加载: ' + CN_FONT_BIN_NAME + ' (' + fontData.length + ' bytes)', 'success');
  } catch (e) {
...
on('fontFlashBtn', 'click', async function() {
  ...
  try {
    ...
    log('写入地址 0x' + CN_FONT_FLASH_BASE.toString(16).toUpperCase() + '（中文字库）', 'info');
    ...
    if (fontData.length !== expectedFontByteLength) {
      log('警告：字库文件 ' + fontData.length + ' 字节，与 CN_FONT_VERSION_OFFSET+1=' + expectedFontByteLength + ' 不一致。将以 bin 实际末尾地址写入版本字节。', 'warn');
    }
    ...
    if (port) await disconnect();
    setTimeout(function() { $('progressContainer').style.display = 'none'; updateProgress(0); }, 1000);
  }
});
...
(function() {
  ...
  var calibSyrupData = null; // 0xB000 v5 固定校准区
  ...
  function renderCalibTable() {
    ...
    CALIB_FIELDS.forEach(function(f) {
      ...
      appendReadonlyCalibCell(tr, calibSyrupData, f);
  ...
  on('calibReadDeviceBtn', 'click', async function() {
    ...
    try {
      ...
      calibThirdPartyData = await readCalibFromDevice(CALIB_V5_BASE, '校准检查-v5/第三方');
      calibSyrupData = calibThirdPartyData;
      updateProgress(100);
      renderCalibTable();
  ...
  function setCalibWriteButtonsDisabled(disabled) {
    var ids = ['calibWriteOfficialBtn', 'calibWriteThirdPartyBtn', 'calibWriteSyrupBtn'];
  ...
  // 写入 v5 校准地址（固定 0xB000）
  on('calibWriteSyrupBtn', 'click', async function() {
    if (isRestoring || isDumping) return;
    if (!calibBackupData && !$('calibCheckBody').querySelector('input[data-offset]')) {
      log(window.t ? window.t('logNoBackupCalibrationWrite') : '没有备份校准数据可写入', 'error');
      return;
    ...
    try {
      ...
      readBuffer = [];
      await sleep(1000);
      await writeCalibToAddress(CALIB_V5_BASE, '写入 v5 校准');
      updateProgress(100);
    } catch(e) {
      log(window.t ? window.t('logError', {msg: e.message}) : '错误: ' + e.message, 'error');
    } finally {
...
/** 写频表导出文件名的前缀 */
const WRITE_FREQ_EXPORT_FILE_PREFIX = 'Syrup';
...
/** settings.c：信道名 @ 0x004000 + ch*16，最多 15 字节 UTF-8/ASCII，整槽写 16 字节（其余 0） */
...
function writefreqDecodeCnNameUtf8(bytes) {
  const len = Math.min(16, bytes ? bytes.length : 0);
  let end = 0;
  let ei = 0;
  for (; ei < len; ei++) {
    const b = bytes[ei];
 ...
 * 按 UTF-8 截断到最多 WRITE_FREQ_CHANNEL_NAME_MAX_BYTES 字节。
 */
function writefreqAsciiChannelNameFromImport(primaryText, englishFallbackText, chineseFallbackText) {
  ...
  return writefreqTruncateUtf8ToMaxBytes(raw, WRITE_FREQ_CHANNEL_NAME_MAX_BYTES).text;
}
...
function writefreqApplyChannelNameBlur(channelNameInput) {
  ...
  const nameResult = writefreqTruncateUtf8ToMaxBytes(raw, WRITE_FREQ_CHANNEL_NAME_MAX_BYTES);
  if (nameResult.wasTruncated) {
    channelNameInput.value = nameResult.text;
    if (typeof showAppToast === 'function') {
      showAppToast(
        '信道名已截断为最长 ' + WRITE_FREQ_CHANNEL_NAME_MAX_BYTES + ' 字节（UTF-8，约 5 汉字）',
        'warn'
      );
    }
  }
...
function writefreqBuildChannelName16(text) {
  // SETTINGS_SaveChannelName：最多 CHANNEL_NAME_MAX_BYTES(15) 的 UTF-8，整槽 16 字节
...
async function writefreqWriteToDevice() {
  ...
  if (nameTruncationWarnings.length > 0) {
    ...
    log(window.t ? window.t('logChannelNameTruncate', {warn: warnJoined}) : '信道名截断提示（UTF-8，最长 15 字节）：\n' + warnJoined, 'warning');
  }
  const messages = [];
  const startCh = writefreqGetBaseChannel();
  let validateRow = 0;
